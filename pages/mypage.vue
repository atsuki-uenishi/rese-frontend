    ...
    methods: {
        async getlike() {
            if(this.$auth.loggedIn) {
                const getData = await this.$axios.get("process.env.VUE_APP_API_URL_BASE/api/v1/users/" + this.$store.state.user.userId + "/likes");
                this.likedStores = getData.data.data
            }
        },
        async getReservations() {
            const getData = await this.$axios.get("process.env.VUE_APP_API_URL_BASE/api/v1/users/" + this.$store.state.user.userId + "/reservations");
        ...
        async like(storeId) {
            ...
            await this.$axios.post("process.env.VUE_APP_API_URL_BASE/api/v1/likes/", sendData);
            } else {
                const likeIndex = this.likedStores.findIndex(like => like.store_id === storeId);
                if(likeIndex !== -1) {
                await this.$axios.delete("process.env.VUE_APP_API_URL_BASE/api/v1/likes/" + this.likedStores[likeIndex].id);
                } else {
                    const sendData = {
                        user_id: this.$store.state.user.userId,
                        store_id: storeId
        ...
        async deleteReservation(reservationId) {
            await this.$axios.delete("process.env.VUE_APP_API_URL_BASE/api/v1/reservations/" + reservationId);
