...
export default {
  ...
  methods: {
    async getStores() {
      const getData = await this.$axios.get("https://rese-atsuki.netlify.app/api/v1/stores");
    ...
    async getlike() {
      if(this.$store.state.user.userId) {
        const getData = await this.$axios.get("https://rese-atsuki.netlify.app/api/v1/users/" + this.$store.state.user.userId + "/likes");
    ...
    async like(storeId) {
      if(!this.likedStores[0]) {
        const sendData = {
          ...
          await this.$axios.post("https://rese-atsuki.netlify.app/api/v1/likes", sendData);
      } else {
        const likeIndex = this.likedStores.findIndex(like => like.store_id === storeId);
        if(likeIndex !== -1) {
          await this.$axios.delete("https://rese-atsuki.netlify.app/api/v1/likes/" + this.likedStores[likeIndex].id);
        } else {
          const sendData = {
              user_id: this.$store.state.user.userId,
              store_id: storeId
